...
class BufferManager{
    ...
    Page insertIntoPool(string matrixName, int xind, int yind);

    public:
    
    BufferManager();
    Page getPage(string tableName, int pageIndex);
    Page getMatrixPage(string matrixName, int indx, int indy);
    ...
    void writeMatrixPage(string matrixName, int indx, int indy, vector<int> &arr);
    void deleteFromPool(string matrixName);
    void emptyPool();
    void printBlockAccessStat();
};
